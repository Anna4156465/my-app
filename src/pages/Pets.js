  ...
  async function deletePet(chip) {
    try {
      ...
      if (response.status === 200) {
        setData(data.filter((item) => item.chip !== chip));
  ...
  return (
    <Box sx={{ p: 4 }}>
          ...
          {data.length > 0 && data.map((row) => {
            return (
              <TableRow hover role="checkbox" tabIndex={-1} key={row.code}>
                {COLUMNS?.map((column) => {
                  const value = row[column.num_chip];
                  return (
                    <TableCell key={column.num_chip} align={column.align}>
      ...
      <Modal open={open} onClose={handleClose}>
        <Box sx={style}>
          {/*Title*/}
          <Typography variant="h6" sx={{ marginBottom: "10px" }}>Create pet</Typography>
          {/*Inputs & buttons*/}
          <form>
